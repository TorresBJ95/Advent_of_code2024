...
def is_safe(data_set: list):
    data_set = list(map(int, data_set))

    es_creciente = all(data_set[i] < data_set[i + 1] for i in range(len(data_set) - 1))
    es_decreciente = all(data_set[i] > data_set[i + 1] for i in range(len(data_set) - 1))

    if not (es_creciente or es_decreciente):
        return False

    return all(1 <= abs(data_set[i] - data_set[i + 1]) <= 3 for i in range(len(data_set) - 1))


def numbers_diference(data: str):
    data_list = read_data_inputs(data)
    counter_saves = 0
    for data_set in data_list:
        ...
        for i in range(len(data_set)):
            temp_set = data_set[:i] + data_set[i + 1:]
            if is_safe(temp_set):
                counter_saves += 1
                break
