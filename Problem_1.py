...
that are historically significant to the North Pole;
...
their list of locations to check is currently empty. Eventually,
...
add up the distances between all the pairs you found. In the example above,
...
how to read most of the Chief's handwriting, but in the commotion
you notice an interesting detail: a lot of location IDs appear in both lists!
...
def read_data_inputs(file: str):
    with open(file, 'r') as file_txt:
        data_lines = file_txt.readlines()
        data_list1 = list()
        data_list2 = list()
        for line in data_lines:
            line = line.split()
            data_list1.append(line[0].rstrip())
            data_list2.append(line[1].rstrip())

        return data_list1, data_list2

def comp_two_number_list(input_path: str):
    list_1, list_2 = read_data_inputs(input_path)
    print(list_1)
    print(list_2)
    min_ordered_list1 = sorted(list_1)
    min_ordered_list2 = sorted(list_2)
    print(min_ordered_list1)
    print(min_ordered_list2)
    dif = 0
    for i in range(len(min_ordered_list1)):
        x = min_ordered_list1[i]
        y = min_ordered_list2[i]
    ...
    # --------------- Part2 -----------------
    result = 0
    for i in min_ordered_list1:
        times_in_list2 = min_ordered_list2.count(i)
        calc = int(i) * int(times_in_list2)
        result += int(calc)
        print(times_in_list2)
    print(result)
